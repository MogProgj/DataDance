...
/**
 * Traced wrapper around {@link DoublyLinkedList}.  Each operation captures a
 ...
 */
public class TracedDoublyLinkedList<T> {
  private final DoublyLinkedList<T> list;
  private final TraceLog log;

  public TracedDoublyLinkedList(DoublyLinkedList<T> list, TraceLog log) {
    this.list = list;
    this.log = log;
  }
  ...
  public void addFirst(T value) {
    ...
    String explanation = wasEmpty
        ...
        : "Added " + value + " at head. New node's next points to old head; old head's prev points to new node.";
  ...
  public void addLast(T value) {
    ...
    String explanation = wasEmpty
        ...
        : "Added " + value + " at tail. New node's prev points to old tail; old tail's next points to new node.";
  ...
  public T removeFirst() {
    ...
    if (list.isEmpty()) {
      log.add(new TraceStep(
          ...
          null, before, before,
          InvariantResult.fromBoolean(list.checkInvariant()),
          "O(1)", "FAILED: Cannot removeFirst from an empty doubly linked list."));
    ...
    String explanation = willBecomeEmpty
        ...
        : "Removed " + value + " from head. Head advanced; new head's prev set to null.";
  ...
  public T removeLast() {
    ...
    if (list.isEmpty()) {
      log.add(new TraceStep(
          list.structureName(), list.implementationName(), "removeLast",
          null, before, before,
          InvariantResult.fromBoolean(list.checkInvariant()),
          "O(1)", "FAILED: Cannot removeLast from an empty doubly linked list."));
      list.removeLast(); // let it throw
    }
    ...
    String after = list.snapshot();
    String explanation = willBecomeEmpty
        ? "Removed " + value + " from tail. List is now empty."
        : "Removed " + value + " from tail. Tail moved back; new tail's next set to null.";
  ...
  public T getFirst() {
    ...
    if (list.isEmpty()) {
      log.add(new TraceStep(
          ...
          null, before, before,
          InvariantResult.fromBoolean(list.checkInvariant()),
          "O(1)", "FAILED: Cannot getFirst from an empty doubly linked list."));
  ...
  public T getLast() {
    ...
    if (list.isEmpty()) {
      log.add(new TraceStep(
          ...
          null, before, before,
          InvariantResult.fromBoolean(list.checkInvariant()),
          "O(1)", "FAILED: Cannot getLast from an empty doubly linked list."));
  ...
  public DoublyLinkedList<T> unwrap() {
    return list;
  }
